...
def print_cm(side_name, correct, total, cm):
    ...
    for i, row in enumerate(cm):
        row_str = f"{STATE_NAMES[i]:>12} | " + " ".join([f"{count:>12}" for count in row])
        print(row_str)
...
def run_test(model_dir, manifest_path, episode_idx=None, show_probs=False, test_all=False):
    ...
    for idx in test_indices:
        ...
        if not test_all: print(f"Testing Episode: {episode['file']}")
        ...
        for side in ["left", "right"]:
            ...
            # Single episode plot
            if not test_all:
                # (Plotting logic same as before, simplified for brevity in this turn)
                pass

    # Print Global Results
    for side in ["left", "right"]:
        if results[side]["total"] > 0:
            print_cm(side, results[side]["correct"], results[side]["total"], results[side]["cm"])

    if not test_all:
        # Re-run single episode with full plotting if only one was requested
...
def test_single_with_plot(model_dir, manifest_path, episode_idx, show_probs):
    ...
    for i, side in enumerate(["left", "right"]):
        ...
        preds, confs = [], []
        for d in df[side]:
            p, pr = engine.update(d); preds.append(p); confs.append(pr)
        preds, confs = np.array(preds), np.array(confs)

        ax_sonar = axes[i]; ax_state = ax_sonar.twinx()
        ...
        ax_state.set_yticks(range(3)); ax_state.set_yticklabels(STATE_NAMES)
        ax_sonar.set_title(f"{side.capitalize()} Episode {episode_idx}")
        if show_probs:
            ax_prob = axes[i+2]
            for s_idx, s_name in enumerate(STATE_NAMES): ax_prob.plot(df['time'], confs[:, s_idx], label=s_name)
            ax_prob.legend()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manifest", type=str, default="manifest.yaml", help="Manifest path")
    parser.add_argument("--episode", type=int, default=0, help="Episode index")
    parser.add_argument("--probs", action="store_true", help="Show probabilities")
    parser.add_argument("--all", action="store_true", help="Test all episodes and show global metrics")
    
    args = parser.parse_args()
    run_test(args.model_dir, args.manifest, args.episode, show_probs=args.probs, test_all=args.all)
