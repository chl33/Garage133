...
from download.download import download_data, parse_time, VALID_STATES
...
def get_state_input(prompt, default_key):
    ...
    while True:
        res = input(f"{prompt} {SHORTCUT_HINT} [{default_state}] (or 'q' to abort): ").strip().lower()
        if res == 'q':
            return 'q'
        if not res:
...
def get_valid_start_time(date_str, timezone):
    ...
    while True:
        ...
        print(f"Error: Could not parse time '{start_hour}'. Please use HH:MM or HH:MM:SS format.")
...
def interactive_session(date_str, timezone, root_dir):
    ...
    print("- TOOLBAR: Use Zoom/Pan buttons to investigate, then toggle them OFF to select")
    ...
    def on_click(event):
        ...
        if event.xdata < t_min or event.xdata > t_max:
            if len(clicks) >= 2:
                ...
                print(f"Ignored click outside data range for {labels[len(clicks)]}")
                return
        else:
    ...
    if left_trans is not None and left_to is not None and left_to != left_initial:
        episode_labels["left"]["transitions"].append({"time": left_trans.strftime('%Y-%m-%dT%H:%M:%SZ'), "to": left_to})
    
    if right_trans is not None and right_to is not None and right_to != right_initial:
        episode_labels["right"]["transitions"].append({"time": right_trans.strftime('%Y-%m-%dT%H:%M:%SZ'), "to": right_to})
    ...
    download_data(final_start, final_end, output_name, "manifest.yaml", episode_labels, root_dir=root_dir)
